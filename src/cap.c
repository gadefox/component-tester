...
uint8_t SetUpDelayTimer(uint8_t Cycles)
{
   ...
   *
   *  NOTE: cycles:
   *  Instruction | ATmega | LGT8FX | Difference
   *   andi          1        1        no
   *   breq          1/2      1/2      no
   *   eor           1        1        no
   *   in            1        1        no
   ...
   */
#ifdef __LGT__
#define CYCLE_OFFSET   33  /* 16 = diff cycles: #ld + #pop*2 + #push*2 + #sbiw + #st*6 + #call + #ret */
#else
#define CYCLE_OFFSET   17
#endif

  if (Cycles < CYCLE_OFFSET)
    return 0;    /* check for required minimum */
...
uint16_t MeasureESR(Capacitor_Type *Cap)
{
  ...
  if (Cap == NULL || CmpValue(Cap->Value, Cap->Scale, 10, -9) < 0)
    return UINT16_MAX;
  ...
  if (Check.Found == COMP_ERROR)
    return UINT16_MAX;   /* skip on error */
  ...
  U_1 = ((MCU_CYCLES_PER_ADC * 25) / 10) - (MCU_CYCLES_PER_US * 10) - (MCU_CYCLES_PER_US * 2) - 10;
...
uint16_t MeasureESR(Capacitor_Type *Cap)
{
  ...
  if ((Cap == NULL) ||
      (CmpValue(Cap->Value, Cap->Scale, 180, -9) < 0))
    return ESR;
...
large_cap:
  ...
  if (U_Cap < 300)
    return 1;                           /* signal too high capacitance */
  ...
  if ((Pulses == 1) && (U_Cap > 1300))
  {
    ...
    /* 1ms pulses (<47µF) */
    return 2;                         /* signal low capacitance (<4.7µF) */
  }

...
#endif  // UI_KEY_HINTS || UI_BATTERY_LASTLINE
...
#endif // UI_SERIAL_COPY || UI_SERIAL_COMMANDS
...
#endif // UI_SERIAL_COMMANDS || SW_DISPLAY_REG
...
#endif // UI_COLORED_TITLES || UI_COLORED_VALUES
...
#endif // FUNC_DISPLAY_COLOREDEESTRING_CENTER
...
{
  /*
   *  Deisplaconvert value into hex digit:
   *  - 0-9: ascii 48-57
   *  - A-F: ascii 65-70
   *    a-f: ascii 97-102
   */
...
#endif // FUNC_DISPLAY_HEXBYTE || FUNC_DISPLAY_HEXVALUE
...
#endif // FUNC_DISPLAY_FULLVALUE || FUNC_DISPLAY_SIGNEDFULLVALUE
...
#endif // FUNC_DISPLAY_SIGNEDFULLVALUE
...
void ChangeContrast(void)
{
  ...
  while (1)
  {
    ...
    Key = TestKey(0, CHECK_KEY_TWICE | CHECK_BAT);

    if (Key == KEY_TWICE)               /* two short key presses */
      return;                           /* end loop */

    if (Key == KEY_SHORT                /* short key press */
#if defined(HW_ENCODER) || defined(HW_INCDEC_KEYS) || defined(HW_TOUCH)
        || Key == KEY_RIGHT              /* rotary encoder: right turn */
#endif
       )
    {
      if (Contrast < Max)
        Contrast++;                     /* increase value */
    }
...
#if defined(HW_ENCODER) || defined(HW_INCDEC_KEYS) || defined(HW_TOUCH)
    if (Key < KEY_RIGHT)                /* just for test button usage */
#endif

...
void CheckDepletionModeFET(uint16_t U_Rl)
{
  ...
  uint8_t           Signal = SIGNAL_NONE; /* control and signal flag */
  ...
  if (U_2 > (U_1 + Offset))   /* exceeds required offset */
  {
    ...
    if (U_1 <= 5)             /* emitter shunt <= 5mV (7µA) */
      Signal = SIGNAL_OTHER;  /* most likely a Darlington BJT */
    ...
    if ((U_1 < 700) &&        /* low V_BE < (550mV + 150mV) */
      ...
      Signal = SIGNAL_OTHER;  /* most likely a Ge BJT */
    ...
    if (U_1 > U_2)              /* valid voltages */
    {
      ...
      if (U_1 < 450)            /* low V_f < 450mV */
        Signal = SIGNAL_OTHER;  /* most likely a Schottky BJT */
    }
    ...
    if (U_1 > 3911)           /* MOSFET */
    {
      ...
      Signal = SIGNAL_FET;                   /* signal match */
    }
    else                                     /* JFET */
    {
      if (Signal == 0)                       /* not a BJT */
      {
  ...
  if (Signal != SIGNAL_FET)             /* no n-ch dep-mode FET found */
  {
    Signal = SIGNAL_NONE;               /* reset control flag */
    ...
    if (U_1 > (U_2 + Offset))      /* exceeds required offset */
    {
      ...
      {
        if (Signal == SIGNAL_NONE)      /* not a BJT */
        {
  ...
  if (Signal == SIGNAL_FET)        /* found depletion-mode FET */
  {
    /* common stuff */
  ...
  if ((U_1 < 1600) && (U_2 > 4400))
  {
    ...
    if (U_1 <= 244)      /* voltage at Anode is low (no current) */
    {
      ...
      if (U_1 < 733)         /* no current -> Thyristor */
      {
        ...
        goto save;                       /* save data and signal success */
      }
      ...
      if (U_2 <= 244)       /* no current */
      {
        ...
        /* wrong pinout */
        return 1;              /* signal success, but don't save data */
      }
    }
  }
...
void CheckUJT(void)
{
  ...
  if (Check.Resistors == 1)
    return;

  /* no resistors found yet */
  ...
  Check.Resistors = 0;           /* reset */

  /* check for 3 - 15 kOhms */
  if (CmpValue(Resistors[0].Value, Resistors[0].Scale, 3000, 0) != 1 ||
      CmpValue(Resistors[0].Value, Resistors[0].Scale, 15000, 0) != -1)
    return;
    ...
    if (U_3 > 1000)         /* more than 1V (1.4mA) */
    {
      ...
      if (Check.AltFound == COMP_UJT) /* second test run */
      {
        ...
        if (AltSemi.U_1 > U_2)        /* larger voltage wins */
          return;         /* keep pinout of first run */
      }

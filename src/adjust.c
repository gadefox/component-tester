...
#endif // CAP_MULTIOFFSET || R_MULTIOFFSET
  ...
  if (Mode != STORAGE_SAVE)        /* read mode */
  {
    ...
    if (*Data_RAM != 0)                /* EEPROM updated */
    {
      if (*Data_RAM != Check)          /* checksum mismatch */
      {
        ...
        return 0;                                 /* signal checksum problem */
      }
    }
  }
...
void ManageAdjustmentStorage(uint8_t Mode, uint8_t ID)
{
  ...
  /* write/read EEPROM */
  if ((DataStorage((uint8_t *)&NV, Data_EE, sizeof(Adjust_Type), Mode) == 0 ||
       DataStorage((uint8_t *)&Touch, (uint8_t *)&NV_Touch, sizeof(Touch_Type), Mode) == 0) &&  /* touch screen offsets */
      Mode == STORAGE_LOAD)
...
uint8_t SelfAdjustment(void)
{
  ...
  while (Step <= 6)      /* loop through steps */
  {
    ...
    while (Loop <= 5)
    {
      ...
      if (Loop < 100)                   /* when we don't skip this test */
      {
        ...
        {
          Loop = 100;                   /* skip current test anyway */
          if (DisplayFlag == KEY_LONG)
            Step = 100;  /* also skip selftest */
        } 
      }
  ...
  Loop = 0;                   /* reset adjustment counter */
  ...
  if (Loop == 4)         /* all adjustments done */
  {

...
uint8_t LC_Meter(void)
{
  ...
  uint8_t           Status;             /* loop control flag */
  uint8_t           Test;               /* user feedback */
  ...
  /* control flags */
  #define UPDATE_MODE         0b00000001     /* update mode (L/C) */
  #define SHOW_VALUE          0b00000010     /* show value */
  #define NO_VALUE            0b00000100     /* no value available */
  ...
  if (Test)                        /* adjustment done */
  {
    ...
    Flag = 1; 
    Status = UPDATE_MODE;               /* set control flags */
  }
  ...
  while (1)
  {
    ...
    if (Status & UPDATE_MODE)
    {
      ...
      Status |= SHOW_VALUE | NO_VALUE;
      Status &= ~UPDATE_MODE;         /* clear flag */
    }
    ...
    switch (Test)
    {
      case KEY_TWICE:                   /* two short key presses */
        /* end tool */
        goto quit;                      /* end processing loop */
      ...
      case KEY_LEFT:
        ...
        Status |= UPDATE_MODE;             /* update mode */
        break;
      ...
      case KEY_LONG:                    /* long key press */
        ...
        Status |= SHOW_VALUE | NO_VALUE;   /* display "no value" */
    }
    ...
    if (Status & SHOW_VALUE)
    {
      ...
      Status &= ~(SHOW_VALUE | NO_VALUE);  /* clear flags */
    }
  }

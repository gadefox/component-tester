...
#if defined(ONEWIRE_IO_PIN) || defined(ONEWIRE_PROBES) || defined(SW_DS18B20) || defined(SW_DS18S20) || defined(SW_ONEWIRE_SCAN)
...
uint8_t OneWire_AddressClient(uint8_t *ROM_Code)
{
  ...
  if (OneWire_ResetBus() == 0)              /* detected client(s) */
    return 0;
...
uint8_t OneWire_Scan_Tool(void)
{
  ...
  if (OneWire_Probes(OneWire_Scan_str) == 0)              /* bus error */
...
#endif  // ONEWIRE_IO_PIN || ONEWIRE_PROBES

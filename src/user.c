...
#endif // HW_BUZZER || BUZZER_PASSIVE
...
uint8_t ReadEncoder(void)
{
  ...
  if (Old_AB != AB)        /* signals changed */
  {
    ...
    if ((Temp & 0b00000001) == 0)       /* B hasn't changed */
...
uint8_t ReadIncDecKeys(void)
{
  ...
  while (1)
  {
    ...
    if (Temp >= 10)                /* long key press (300ms) */
      break;                       /* end loop & signal long key press */
  ...
  {
    TicksInc = 1;                  /* default step size */

    if (Key == UI.KeyOld)          /* same key as before */
    {
      if (Temp != 0)               /* long button press */
      {
        TicksInc = UI.KeyStepOld;  /* get former step size */

        if (TicksInc <= 6)         /* limit step size to 7 */
          TicksInc++;              /* increase step size */
      }
    }

    UI.KeyStepOld = TicksInc;      /* update former step size */
    UI.KeyStep = TicksInc;         /* set new step size */
  }
...
uint8_t TestKey(uint16_t Timeout, uint8_t Mode)
{
  ...
  if (Mode & CHECK_OP_MODE)             /* consider operation mode */
  {
    if (Cfg.OP_Mode & OP_AUTOHOLD)      /* in auto-hold mode */
      Timeout = 0;                      /* disable timeout; and keep cursor mode */
    else                                /* in continuous mode */
      Mode &= ~(CURSOR_STEADY | CURSOR_BLINK);    /* clear all cursor flags; and keep timeout */
  }
  ...
  while (Run)
  {
      ...
      while (Run)             /* detect how long key is pressed */
      {
        ...
        if (Test == 0)        /* key still pressed */
        {
          ...
          if (Ticks > 26)
            ...
            MilliSleep(10);             /* otherwise wait 10ms */
        }
        else                            /* key released */
          Run = 0;                      /* end loop */
      }
    ...
    else                      /* no key press */
    {
      ...
      if (Test)                    /* got user input */
      {
        ...
        if (Test != Key)           /* changed direction */
          break;                   /* exit loop; keep last direction and velocity level #1 */
      ...
      {
        if (UI.EncTicks >= Timeout2)  /* timeout for velocity detection; keep velocity level #1 */
          break;                      /* exit loop */
      }
#endif // HW_ENCODER
      ...
      if (Ticks % DELAY_100 == 0)       /* every 100ms */
      {
        ...
        else                            /* timeout triggered */
        {
          if (Mode & CHECK_BAT)         /* battery check requested */
            CheckBattery();             /* check battery; also powers off on low battery */
        }
      } 
#endif // BAT_NONE
...
uint8_t MenuTool(uint8_t Items, uint8_t Type, void *Menu[], unsigned char *Unit)
{
  ...
  while (Run)
  {
    ...
    while (n < Lines)
    {
      ...
      if (n > Items)
        n = Lines;    /* end loop for a short list */
    }

...
void LCD_SendNibble(uint8_t Nibble)
{
  ...
  if (Nibble & 0b00000001)
    Data |= (1 << LCD_DB4);
  if (Nibble & 0b00000010)
    Data |= (1 << LCD_DB5);
  if (Nibble & 0b00000100)
    Data |= (1 << LCD_DB6);
  if (Nibble & 0b00001000)
    Data |= (1 << LCD_DB7);
#endif
...
void LCD_BusSetup(void)
{
  ...
  SPI_ClockRate = SPI_CLOCK_R0 | SPI_CLOCK_2X;
#elif F_CPU == 20000000
  /* 20MHz -> f_osc/8 (SPR1 = 0, SPR0 = 1, SPI2X = 1) */
  SPI_ClockRate = SPI_CLOCK_R0 | SPI_CLOCK_2X;
#elif F_CPU == 24000000
  /* 24 MHz -> f_osc/16 = 1.5 MHz (SPR1 = 1, SPR0 = 0, SPI2X = 0) */
  SPI_ClockRate = SPI_CLOCK_R1;
#elif F_CPU == 32000000
  /* 32 MHz -> f_osc/16 = 2.0 MHz (SPR1 = 1, SPR0 = 0, SPI2X = 0) */
  SPI_ClockRate = SPI_CLOCK_R1;
#else
...
void LCD_Char(unsigned char Char)
{
  ...
  if (Index == 0xff)
    return;            /* no character bitmap available */
...
void LCD_Char(unsigned char Char)
{
  ...
  /* update char matrix */
  if (StepFlag)
    Buffer++;          /* move one char to the right */
  else
    Buffer--;                   /* move one char to the left */
  *Buffer = Char;                  /* set new char */

  /* update character position */
  UI.CharPos_X++;                  /* next character in current line */
  if (StepFlag)
    X_Start--;         /* also update X dot position */
}

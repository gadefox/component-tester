...
void Touch_SelectChip(void)
{
  ...
  OldClockRate = SPI_ClockRate;         /* save old clock settings */
  SPI_ClockRate = ClockRate;            /* set new clock rate */
...
void Touch_DeselectChip(void)
{
  ...
  SPI_ClockRate = OldClockRate;         /* set old clock rate */
  SPI_Clock();                          /* update SPI clock */
#endif
}

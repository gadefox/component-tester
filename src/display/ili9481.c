...
#if (defined(SPI_BITBANG) || defined(SPI_HARDWARE)) && !defined(SPI_9)
...
void LCD_BusSetup(void)
{
  ...
  SPI_Clock(SPI_CLOCK_2X);         /* update SPI clock */
#endif // SPI_HARDWARE
}
...
void LCD_Data2(uint16_t Data)
{
  ...
  /* send data: 2 bytes */
  SPI_Write_Byte(Data >> 8);       /* write MSB */
  SPI_Write_Byte(Data);            /* write LSB */
...
#endif // (SPI_BITBANG || SPI_HARDWARE) && !SPI_9
...
void LCD_ClearLine(uint8_t Line)
{
  ...
  if (Line <= 16)                  /* prevent overflow */
  {
    ...
    if (Pos == 1)             /* bit set and complete line */
    {
